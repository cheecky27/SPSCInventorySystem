...
public class UpdateUserPasswordDaoImpl implements UpdateUserPasswordDao {

	@Override
	public User getUserPassword(String una) {
		DatabaseManager dm = new MySQLDatabaseManager();
		Connection conn = dm.getConnection();
		User myUser = new User();
		StringBuilder sql = new StringBuilder(300);
		
		sql.append("UPDATE `inventory system`.`user` SET `passWord`=? WHERE `iduser`='1' ");
